...
var (
	...
	astPkgList         []*ast.Package
	importList         map[string]string
	controllerComments map[string]string
	modelsList         map[string]map[string]swagger.Schema
	controllerList     map[string]map[string]*swagger.Item //controllername Paths items
)

type Param struct {
	CurrentPath    string
	RouterPath     string // routers
	RouterGo       string //router.go
	OutPutJsonPath string
	OutPutYmlPath  string
}

func NewParm(currentPath, routerPath, routerGo, outPutJsonPath, outPutYmlPath string) *Param {
	if routerPath == "" {
		routerPath = "routers"
	}
	if routerGo == "" {
	...
	if outPutJsonPath == "" {
		outPutJsonPath = "swagger/swagger.json"
	}
	if outPutYmlPath == "" {
		outPutYmlPath = "swagger/swagger.yml"
	}
	return &Param{
		...
		OutPutYmlPath:  outPutYmlPath,
	}
}
func GenerateDocs(par *Param) {
	parsePackagesFromDir(par.CurrentPath)
	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, filepath.Join(par.CurrentPath, par.RouterPath, par.RouterGo),
		nil, parser.ParseComments)
	if err != nil {
		logger.Fatalf("Error while parsing %s: %v", par.RouterGo, err)
	}
		...
		for _, c := range f.Comments {
			for _, s := range strings.Split(c.Text(), "\n") {
				if strings.HasPrefix(s, "@APIVersion") {
					...
					if len(p) < 2 {
						logger.Fatalf("Not enough params for security: %d", len(p))
					}
					out.Type = p[1]
					switch out.Type {
					case "oauth2":
						if len(p) < 6 {
							logger.Fatalf("Not enough params for oauth2: %d", len(p))
						}
						if !(p[3] == "implicit" || p[3] == "password" || p[3] == "application" || p[3] == "accessCode") {
							logger.Fatalf("Unknown flow type: %s. Possible values are `implicit`, `password`, `application` or `accessCode`.\n", p[1])
						}
					...
					case "apiKey":
						if len(p) < 4 {
							logger.Fatalf("Not enough params for apiKey: %d", len(p))
						}
						if !(p[3] == "header" || p[3] == "query") {
							logger.Fatalf("Unknown in type: %s. Possible values are `query` or `header`.", p[4])
						}
						out.Name = p[2]
						out.In = p[3]
						if len(p) > 4 {
					...
					default:
						logger.Fatalf("Unknown security type: %s. Possible values are `oauth2`, `apiKey` or `basic`.\n", p[1])
					}
	...
	jsonPath := path.Join(par.CurrentPath, par.OutPutJsonPath)
	os.MkdirAll(path.Dir(jsonPath), 0755)
	ymlPath := path.Join(par.CurrentPath, par.OutPutYmlPath)
	os.MkdirAll(path.Dir(ymlPath), 0755)
	fd, err := os.Create(jsonPath)
	if err != nil {
		logger.Panic(err.Error())
	}
	fdyml, err := os.Create(ymlPath)
	if err != nil {
		logger.Panic(err.Error())
	}
	defer fdyml.Close()
...
func parsePackageFromDir(path string) error {
	folderPkgMap, err := parser.ParseDir(token.NewFileSet(), path, func(info os.FileInfo) bool {
		name := info.Name()
	...
	for _, v := range folderPkgMap {
		astPkgList = append(astPkgList, v)
	}
	return nil
}
...
func getSecurity(t string) (security map[string][]string) {
	...
	if len(p) == 0 {
		logger.Fatalf("No params for security specified")
	}
...
func analyseControllerPkg(localName, pkgPath string) {
	...
	if err != nil {
		logger.Fatalf("Package %s cannot be imported: %v", pkgPath, err)
	}
	pkgRealpath := pkg.Dir
	if pkgRealpath != "" {
		...
		logger.Fatalf("Package '%s' does not have source directory", pkgPath)
	}
		...
		for _, c := range comments.List {
			...
			if strings.HasPrefix(t, "@router") {
				...
				case "body":
					break
				default:
					logger.Warnf("[%s.%s] Unknown param location: %s. Possible values are `query`, `header`, `path`, "+
			...
			for _, fl := range pkg.Files {
				for k, d := range fl.Scope.Objects {
					if d.Kind == ast.Typ {
						...
						parseObject(d, k, &m, &realTypes, astPkgList, packageName)
...
func parseObject(d *ast.Object, k string, m *swagger.Schema, realTypes *[]string, astPkgs []*ast.Package, packageName string) {
	...
	if !ok {
		logger.Fatalf("Unknown type without TypeSec: %v", d)
	}
		...
		for _, fl := range pkg.Files {
			for _, obj := range fl.Scope.Objects {
				if obj.Kind == ast.Con {
					...
					if !ok {
						logger.Fatalf("Unknown type without ValueSpec: %v", vs)
					}
					...
					for i, val := range vs.Values {
						...
						if !ok {
							logger.Warnf("Unknown type without BasicLit: %v", v)
							continue
						}
						...
						case token.INT:
							...
							if err != nil {
								logger.Warnf("Unknown type with BasicLit to int: %v", v.Value)
								continue
							}
...
func analyseNSInclude(baseurl string, ce *ast.CallExpr) string {
	...
	for _, p := range ce.Args {
		...
		if _, ok := p1.(*ast.UnaryExpr); ok {
			...
			logger.Warnf("Couldn't determine type\n")
			continue
		}
